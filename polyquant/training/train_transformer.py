#!/usr/bin/env python
"""
Training script for the MarketTransformer model.

Uses the sequence dataset (MarketWindowDataset) which provides variable-length
trade sequences per market for binary outcome prediction.
"""
...
from polyquant.models.transformer import create_base_transformer, create_small_transformer
...
def main():
    ...
    # Model
    model = create_base_transformer(
        d_input=10,
        max_seq_len=SEQ_LEN,
    ).to(device)
    print(f"[INFO] Model parameters: {model.count_parameters():,}")

    # Optimizer (no built-in scheduler, we use manual LR)
    ...
    if args.resume:
        ...
        run_name = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S") + "_transformer"
        global_step = 0
        epoch = 0
